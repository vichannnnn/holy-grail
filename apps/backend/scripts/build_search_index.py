...
Usage:
    cd apps/backend
    uv run python scripts/build_search_index.py [--recreate] [--limit N]
...
async def build_index(recreate: bool = False, limit: int | None = None) -> None:
...
def main() -> None:
    ...
    asyncio.run(build_index(recreate=args.recreate, limit=args.limit))
